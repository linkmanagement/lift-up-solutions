...
const ERROR = {
    ...
    TELEGRAM: 'Telegram handle is required',
    NUMBER: 'Number is Required!',
    INVALID_NUMBER: 'Only numbers are allowed',
    AMOUNT_OF_MODELS: 'Amount of models is Required!',
...
function getNextStage(stage) {
    switch (stage) {
        case STAGE.PROMPT:
            return STAGE.NAME;
        case STAGE.NAME:
            return STAGE.TELEGRAM;
        case STAGE.TELEGRAM:
            return STAGE.NUMBER;
        case STAGE.NUMBER:
            return STAGE.AMOUNT_OF_MODELS;
        case STAGE.AMOUNT_OF_MODELS:
...
function Telegram({ stage, setStage, telegram, setTelegram, error, setError, completedStages }) {

    return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
            <h1 className="text-2xl font-regular md:w-[60vw]">
                {`Please provide your Telegram handle below.*`}
            </h1>
            <div className="mt-2 flex flex-col gap-4 items-center">
                <p className="text-red-600 text-sm opacity-70">
                    {error}
                ...
                <input autoFocus type="text" className="mt-4 px-4 py-2 bg-gray-100 rounded border border-blue-300"
                    ...
                    onChange={(e) => { setError(ERROR.NONE); setTelegram(e.target.value); }} value={telegram} />
                <div className='flex items-center gap-2' >
...
function Number({ stage, setStage, number, setNumber, error, setError, completedStages }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
            <h1 className="text-2xl font-regular md:w-[60vw]">
                {`Please provide your Telegram number below.*`}
            </h1>
            <div className="mt-2 flex flex-col gap-4 items-center">
                <p className="text-red-600 text-sm opacity-70">
                    {error}
...
function Earnings({ stage, setStage, earnings, setEarnings, error, setError, completedStages, setIsLoading, name, telegram, number, onlyfans }) {

    return (
        <div className="flex flex-col items-center justify-center flex-1 text-center">
            <h1 className="text-2xl font-regular md:w-[60vw]">
                {`Please provide your Agency Revenue (per month) below.*`}
            </h1>
            <div className="mt-2 flex flex-col gap-4 items-center">
                <p className="text-red-600 text-sm opacity-70">
                    {error}
                ...
                <div className='flex items-center gap-2' >
                    ...
                    <button className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded" onClick={async () => {
                        if (earnings === '') {
                            ...
                            setStage(getNextStage(stage));
                            // setIsLoading(true);
                            // await addLead({ name, telegram, number, onlyfans, earnings });
                            // setIsLoading(false);
                        }
                    }}>
                        SUBMIT
...
function StageNavigator({ stage, setStage, completedStages }) {
    ...
    // Define stages
    let stages = [STAGE.NAME, STAGE.TELEGRAM, STAGE.NUMBER, STAGE.ONLYFANS, STAGE.EARNINGS];
...
export default function Apply() {
    ...
    const [amountOfModels, setAmountOfModels] = useState('');
    ...
    useEffect(() => {
        ...
        console.log('Instagram:', telegram);
        ...
        , [stage, name, telegram, number, onlyfans, earnings, error]);
    ...
    useEffect(() => {
        ...
        function handleKeyDown(e) {
            ...
            if (e.keyCode == 13) {
                ...
                else if (stage === STAGE.TELEGRAM && telegram === '') {
                    console.log('Instagram Handle is empty');
                    setError(ERROR.TELEGRAM);
                }
    ...
    return (
        <div className="h-screen flex flex-col items-center justify-center p-6">
            ...
            {
                stage === STAGE.TELEGRAM &&
                <Telegram stage={stage} setStage={setStage} telegram={telegram} setTelegram={setTelegram} error={error} setError={setError} completedStages={completedStages} />
            }
            {
                stage === STAGE.NUMBER &&
